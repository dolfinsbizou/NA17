...
function Parcelle_get_all($join=false)
{
	...
	$req = $db->query('SELECT id, sol,' . ($join?' TypeSol.description AS sol_desc,':'') . ' exposition, superficie FROM Parcelle' . ($join?' INNER JOIN TypeSol ON Parcelle.sol = TypeSol.nom':'') . ' ORDER BY id');
